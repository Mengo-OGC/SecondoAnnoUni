...
\begin{itemize}
	...
	\item \textbf{Carattere Qualitativo} se non è quantitativo.
\end{itemize}
...
\definizione{Campione}{Un campione è un sotto insieme (rappresentativo) della popolazione del quale possiamo determinare il valore.}{d:campione}

\definizione{Modalità}{La modalità sono i valori che può assumere il carattere.}{d:modalità}

\definizione{Classi}{Una classe, solitamente, è determinata dal \textbf{confine inferiore e superiore}.
...
Se le modalità sono molto numerose (o infinite) è conveniente raggrupparle in \textbf{classi}.
...
\definizione{Linearità della media}{Siano $x,y,z$ tre caratteri legati dalla relazione $z=ax+by$ con costanti $a,b$. Allora \[\overline{z}=a\overline{x}+b\overline{y}\]}{d:linMed}
...
\definizione{Media Ponderata}{La media ponderata si usa per dare diversa importanza (peso) ai vari elementi di un campione. \[\overline{x_w}=\frac{x_1w_1+\dots+x_nw_n}{w_1+\dots+w_n}\]}{d:medPond}
...
Possiamo fissare un punto a caso $t$. Consideriamo la media dei quadrati delle distanze date come: \[\frac{(x_1-t)^2+\dots+(x_n-t)^2}{n}\] Per quale $t$ questa quantità è minima?
\begin{align*}
	\frac{1}{n}\sum_{i=1}^n(x_i-t)^2&=\frac{1}{n}(\sum_{i=1}^n(t^2-2x_1t+x_1^2))\\
	&=t^2-\frac{2t}{n}\sum_{i=1}^nx_i+\frac{1}{n}\sum_{i=1}^nx_i^2\\
	&=t^2-2\overline{x}t+\frac{1}{n}\sum_{i=1}^nx_i^2
\end{align*}
...
\proprieta{Prima proprietà varianza}{La varianza rimane invariata per traslazioni di costanti. \[\sigma_y^2=\sigma_x^2\]}{prop:var1}

\dimostrazione{prop:var1}{Sia $y=x+c$ con $c$ costante. Otteniamo $\overline{y}=\overline{x}+c$ ovvero $\overline{y^2}=\overline{x^2+2cx+c^2}=\overline{x^2}+2c\overline{x}+c^2$ che ci da
\[\sigma_y^2=\overline{y^2}-\overline{y}^2=\overline{x^2}+2c\overline{x}+c^2-(\overline{x}^2+2c\overline{x}+c^2)=\sigma_x^2\]}

\proprieta{Seconda proprietà varianza}{La varianza aumenta per prodotto con costanti. \[\sigma_y^2=a^2\sigma_x^2\]}{prop:var2}
...
\definizione{Covarianza Campionaria}{La covarianza campionaria di $x$ e $y$ è \[\sigma_{x,y}=\frac{1}{n}\sum_{i=1}^n(x_i-\overline{x})(y_i-\overline{y})=\overline{xy}-\overline{x}\cdot \overline{y}\]}{d:covarianza}
...
\dimostrazione{d:covarianza}{\begin{align*}
	\sigma_{x,y}&=\frac{1}{n}\sum_{i=1}^n(x_i-\overline{x})(y_i-\overline{y})\\
	&=\frac{1}{n}\sum_{i=1}^n(x_iy_i-x_i\overline{y}-\overline{x}y_i+\overline{x}\cdot\overline{y})\\
	&=\overline{xy}-\cancel{\overline{x}\cdot\overline{y}}-\overline{x}\cdot\overline{y}+\cancel{\overline{x}\cdot\overline{y}}\\
	&=\overline{xy}-\overline{x}\cdot \overline{y}\\
\end{align*}}

\definizione{Retta ai minimi quadratici}{La retta ai minimi quadratici è la retta che minizza i quadrati degli errori (cioè delle lunghezze dei segmenti verticali).}{def:rettaMinQuad}
La somma dei quadrati degli errori è \[S(m,q)=\sum_{i=1}^n(y_i-mx_i-q)^2\] vogliamo $m$ e $q$ in modo che questa quantità sia minima. \[S(m,q)=\sigma_y^2+m^2\sigma_x^2-em\sigma_{x,y}+(\overline{y}-q-m\overline{x})^2\]
\begin{center}
	\begin{tikzpicture}
...
\definizione{Media geometrica}{La media geometrica permette di ottenere la variazione percentile media dei fattori moltiplicativi: \[\overline{x_g}=\sqrt[n]{x_1\cdot\ldots\cdot x_n}\] \[\overline{x_g}^n=x_1\cdot\ldots\cdot x_n\]}{d:medGeom}
