...
\definizione{S.S.I. senza rimpiazzo}{Sia $\Omega=$ le combinazioni (sotto insiemi), Con probabilità uniforme, di $n$ palline tra blu ($b$) e rosse ($r$). $|\Omega|=\binom{b+r}{n}$.
...
Diciamo $T\sim \tilde{G}(p)$, ossia $T$ ha densità geometrica modificata di probabilità $p$.
...
\osservazione{$d(k)$ è una densitù discreta astratta:
...
\teorema{Variabile di Poisson}{Sia $X$ una variabile binomiale, $X\sim B(n,p)$, e poniamo $\lambda=pn$ ottentendo $XX\sim B(n, \lambda n)$.
...
\definizione{Variabili aleatorie multidimensionali}{Una variabile aleatoria n-dimensionale è data da $(X_1,\dots,X_n)$ dove $X_1,\dots,X_n$ sono variabili aleatorie.}{d:vam}
