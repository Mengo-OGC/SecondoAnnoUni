...
La valutazione di probabilità, $P:\mathscr{A}\to\mathbb{R}_{\geqslant0}$, deve soddisfare le proprietà:
\begin{enumerate}
	\item $P(\Omega)=1$.
...
È una formula di inversione della condizione, $P(B\mid A)\leftrightarrow  P(A\mid B)$.
...
La densità concreta discreta di $X$ è $dx:\mathbb{R}\to\mathbb{R}$ con \[d_X(h)=P(X=h)=
\begin{cases}
	a_1 \ se\ h=0,\dots \\
	\dots \\
	0 \ altrimenti
...
Il tempo di primo successo in uno s.s.i. a prove indipendenti definisce $T$ come Tempo primo successo che assume $\infty$ valori e $p$ come Probabilità di avere successo ad'ogni tentativo.
...
$X+1=T$ è una variabile geometrica modificata con $X$ che assume valori da 0 a $+\infty$. Quindi
...
Le dens.marginali sono univocamente determinabili dalla dens.congiunta:
