...
\definizione{Densità uniforme}{Sia $X$ una variabile aleatoria.

Si dice uniforme nell'insieme $A=\{a_1,\dots,a_n\}\subseteq\mathbb{R}$ se \[dx(h)=
\begin{cases}
...
La variabile $X=\# successi$ la chiamiamo binomiale e scriviamo $X~B(h,p)$ con $h$ il numero di tentativi e $p$ la probabilità che ogni tentativo dia successo.
...
\definizione{S.S.I. senza rimpiazzo}{Sia $\Omega=$ combinazioni (sotto insiemi) di $n$ palline scelte tra le $b+r$ (blu e rosse) disponibili. \[|\Omega|=\binom{b+r}{n}\]
Con probabilità uniforme.

Uno schema successo insuccesso si dice senza rimpiazzo se:
\begin{enumerate}
...
La variabile $X=\# successi$ (estrazione pallina blu) la chiamiamo ipergeometrica e scriviamo $X\sim H(h,b,r)$ con $h$ il numero di tentativi.

I risultati favorevoli a "$X=k$": $\binom{b}{k}\cdot\binom{r}{h-k}$:
Ossia la scelta di $k$ palline blue per la scelta di $h-k$ palline rosse.
...
\definizione{Variabili tempo di primo successo}{Sia $\Omega=$ combinazioni (sotto insiemi) di $n$ palline scelte tra le $b+r$ (blu e rosse) disponibili.

Se estraiamo senza rimpiazzo (condizionati) fino ad'ottenere un successo, cioè un blu.
Con $T=\# tentativi \ effettuati$. }{d:varTempPrimoSucc}

Il tempo di primo successo in uno s.s.i. a prove indipendenti definisce:
\begin{enumerate}
	\item $T$=Tempo primo successo assume $\infty$ valori.
...
Diciamo che $T$ ha densità geometrica modificata di probabilità $p$:
...
$X+1=T$ è una variabile geometrica modificata con $X$ che assume vlaori da 0 a $+\infty$.
