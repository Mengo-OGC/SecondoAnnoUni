...
\definizione{Ssi a prove indipendenti}{Uno schema successo insuccesso si dice a prove indipendenti se:
	...
	\end{enumerate}
	La variabile $X=\# successi$ la chiamiamo binomiale. Scriviamo $X\sim B(n,p)$ con $h$ il numero di tentativi e $p$ la probabilità che ogni tentativo dia successo.
	\[d_X(k)=
	\begin{cases}
...
\begin{aligned}
	...
	&= P(X=n)
\end{aligned}\]
Le marginali sono univocamente determinabili dalla congiunta:
\[d_X(h)=\sum_kd_{X,Y}(h,k)\quad d_Y(k)=\sum_hd_{X,Y}(h,k)\]
