...
\subsection{JUnit}
Crea classi di test tramite metodi annotati che testano scenari per asserire la correttezza del  risultato rispetto a un valore atteso.

\multiline{}{}{CodiciEsempio/test.java}

\begin{itemize}
	\item Un test va a buon fine se non fallisce nessun assert (o è interrotto da \inline{return}).

	\item \inline{fail()} fa fallire il test "forzatamente".

	\item L'annotazione \inline{\@BeforeEach} fa eseguire un metodo prima di ogni test.
\end{itemize}

Eseguire \inline{./gradlew build} comporta il check (controlli + test(compileTestJava + compileJava)) e asseble (jar + compileJava).

Le cartelle diventano: \inline{/build, /src/main, /src/test}.
...
\subsection{Javadoc}
La documentazione semi-automatica (HTML) è generata dal componente \inline{javadoc} traminte una specifica sintassi dei commenti.

\textbf{Sintassi}:
I commenti del tipo \inline{/** ... */} e in testa a classi/interfacce/membri, possono contenere i tag:
\begin{itemize}
