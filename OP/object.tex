...
\oneline{javac -d "<dirEseguibili>" [-cp "<p1[:|;...]>"] <elencoSorgenti>}

\oneline{java -cp "<p1[:|;...]>" <classeMain>}

\begin{itemize}
	\item \inline{-d} specifica al compilatore la cartella dove creare i .class. Se non già presente la crea.
	\item \inline{-cp} linka una o più (divise da : o ;) classi del classpath\footnote{Sottopercorso (insieme ordinato) di cartelle che corrisponde al percorso del package dichiarato per la classe corrispondente. Il classpath di default è il JRE (Java Runtime Environment).} indicato a quello che si genererà.
