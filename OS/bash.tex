...
\tabella{Directory}{
	...
	\riga{ls -a [\textit{nomefile}]}{Mosta anche file nascosti (anche ., ..).}
	\riga{ls -l [\textit{nomefile}]}{Mostra informazioni file e relativi permessi nel formato: 1°carattere tipo file (- file, d dyrectory, c collegamento seriale, b device a blocchi), 9 caratteri successivi terzine di permessi (r read,w write,x/s execute) per proprietario, gruppo, altri utenti.}
	\riga{ls -h [\textit{nomefile}]}{Rende i dati più leggibili.}
	\riga{ls -d [\textit{nomefile}]}{Fa applicare il comando alla directory stessa/specificata e non ai file.}
	\riga{ls -R [\textit{nomefile}]}{Mostra ricorsivamente contenuto sotto directory.}
...
\tabella{Uso File Descriptor}{
	...
	\riga{program \textit{modalitàEfd} file\textbar prog2 ...}{Ridireziona il file o prog su uno specifico fd di program.
		Modalità: N$>$, N$>>$, $<$N.
	}
...
\tabella{Utilities}{
	...
	\riga{find  \textit{file} -type d}{Trova solo directory.}
	\riga{find \textit{file} -type f}{Trova solo file.}
	\riga{find \textit{file} -maxdepth n}{Limità la profondità di ricerca il numero di livelli indicato.}
	\riga{find \textit{file} -mindepth n}{Indica il livello minino di ricerca.}
	\riga{find \textit{file} -name "..."}{Definisce un vincolo sul nome del file da trovare.}
	\riga{find \textit{file} -iname}{Disabilità il caseSensitive.}
