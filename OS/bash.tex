...
\tabella{Process Identifier}{
	...
	\texttt{\$!} ha il pid del processo lanciato in bg (anche se sganciato). Non è aggiornata se faccio bg di un processo sospeso da \texttt{\^Z}.}
	\riga{jobs}{Mostra lista procesi backgroud e sospesi con rispettivo indice di job.

	Con \texttt{\%indiceJob} si gestisce il job.}
	\riga{\^Z}{Sospende processo in foreground. Lancia il segnale SIGTSTP}
	\riga{\^C}{Termina processo in foreground.  Lancia il segnale SIGINT.}
