...
\tabella{Directory}{
	...
	\riga{ls \textit{nomeDirOpzionale}}{Visualizza i files/direcotry contenuti nella directory corrente o di quella specificata.}
	\riga{ls -a \textit{nomeDirOpzionale}}{Mosta anche file nascosti (anche ., ..).}
	\riga{ls -l \textit{nomeDirOpzionale}}{Mostra informazioni file e relativi permessi nel formato: 1°carattere tipo file (- file, d dyrectory, c collegamento seriale, b device a blocchi), 9 caratteri successivi terzine di permessi (r read,w write,x/s execute) per proprietario, gruppo, altri utenti.}
	\riga{ls -h \textit{nomeDirOpzionale}}{Rende i dati più leggibili.}
	\riga{ls -d \textit{nomeDirOpzionale}}{Fa applicare il comando alla directory stessa/specificata e non ai file.}
	\riga{ls -R \textit{nomeDirOpzionale}}{Mostra ricorsivamente contenuto sotto directory.}
...
\tabella{Tilde Expansion}{
	\riga{$\sim$/...}{Tilde espansa con il percorso assoluto della home directory dell'effective user. Valido caso con solo \textbf{$\sim$} e solo \textbf{$\sim$/}.}
	\riga{$\sim$userName/...}{Tilde e userName espansi con il percorso assoluto della home directory dell'utente specificato. Valido caso con solo \textbf{$\sim$userName/}.}
}
...
\tabella{Uso File Descriptor}{
	...
	\riga{program \textit{modalitàEfd} file\textbar prog2 ...}{Ridireziona il file o prog su uno specifico fd N di program.
		Modalità: N$>$, N$>>$, $<$N.
	}

	\riga{program 0/1/2/\&$>$ file\textbar\&0/1/2}{Ridireziona lo std (\&=stdout+stderr) indicato di program nel file indicato sovrascrivendo o in un altro str. \&$>$ ridireziona solo in fle.}
...
\tabella{Utilities}{
	...
	\riga{sed \textit{'s/old/new/[g\textbar n];...' file}}{Sostituisce il testo old (o più testi orinali con [o1, o2, ...]) con quello new, solo per la prima occorrenza. Può richiere l'opzione \texttt{-r}.
	\begin{itemize}[noitemsep, topsep=0pt]
		\item Con l'opzione g si applica per tutte le occorrenze in una riga oppure all'n-esima occorrenza.

		Si possono  concatenare più sostituzioni d'applicare sulla stessa riga.
		\item \textbf{\^} significa inizio linea. Va all'inizio di new.
		\item \textbf{\$} significa fine riga. Va alla fine di new.
	...
	\riga{find \textit{path}}{Trova percorsi di file in sotto alberi della memoria del path indicato.}
	\riga{find \textit{path} -type d}{Trova solo directory.}
	\riga{find \textit{path} -type f}{Trova solo file.}
	\riga{find \textit{path} -maxdepth n}{Limità la profondità di ricerca il numero di livelli indicato (0 si limita alla cartella indicata).}
	\riga{find \textit{path} -mindepth n}{Indica il livello minino di ricerca (1 è la cartella indicata).}
	\riga{find \textit{path} -name "..."}{Definisce un vincolo sul nome del file da trovare.}
	\riga{find \textit{path} -iname}{Disabilità il caseSensitive.}
	\riga{find \textit{path} -exec ... '\{\}' ... $\backslash$;}{Istruzioni da fare quando viene trovato un elemento. L'output di find viene messo al posto di '\{\}'.}
	\riga{find \textit{path} -print}{Obbliga la stampa dei file trovati anche quando find non stamperebbe.}

	\riga{wc}{Dice del testo passato il numero di righe (-l), parole (-w) e caratteri (-c).}

	\riga{tee file}{Stampa a video e salva l'output di un comando (che gli manda l'input, tipo con |), sul file indicato.}
	\riga{sort \textit{file}}{Ordina alfabeticamente le righe del file.}
	\riga{sort -k n}{Ordina le righe alfabeticamente secondo la colona di stringhe specificata.}
	\riga{uniq \textit{file}}{Rimuove le righe duplicate consecutive. (Bisogna sortare prima).}
	\riga{uniq -c \textit{file}}{Rimuove le righe duplicate consecutive e conta quante sono.}
}
...
\tabella{Espansione Parametri}{
	...
	\riga{\$\{\textit{variabile}/pattern/string\}}{Sostituisce la prima occorrenza da sinistra del pattern con la stringa indicata.
...
\tabella{Gestione risorse}{
	...
	\riga{sudo mkfs.\textit{tipoFileSystem}}{Crea il file system dentro una partizione.}
