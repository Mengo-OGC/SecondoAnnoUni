...
\textbf{MAC}: Si attua combinando crittografia simmetrica e funzioni Hash crittografiche. Attori devono conoscere la chiave segreta. Assente autenticazione di chi spedisce (potrebbe essere l'altro o sestesso). SI: Integrità, autenticazione.
...
\tabella{GPG}{
	\riga{sha256sum \textit{nomefile[, ...]}}{Stampa una riga per ogni file pssato contenete il digest e il file separati da: '*' se file binario, ' ' se file di testo, 'U' se file universale, '\^' per bits.}
	\riga{sha256sum -c \textit{nomeFile.sha256}}{Verifica correttezza del digest contenuto in nomeFile.sha256.

	Il rispettivo nomeFile deve essere nella direcotry corrente.}
	...
	\riga{gpg --armor --detach-sign nomefile}{Crea file di firla nomefile.asc (firma detached).}
