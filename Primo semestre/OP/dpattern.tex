...
Idea: Fare le \inline{new} in modo controllato usando dei metodi muniti di nome esplicativo di ciò che vanno a creare.
...
\textbf{Famiglia Factory (Oggetto)}: Risolve il problema di decidere quale oggetto creare.
\begin{itemize}
	\item \textbf{Static Factory}: Classe con metodo statico che restituisce un'istanza della classe stessa o di una sua specializzazione.

	\item \textbf{Simple Factory}: Classe con metodo non statico che genera instasnza d'un altra calsse Target.

	\item \textbf{Factory Method}: Classe, implementa interfaccia con metodono non statico, che genera instanze delle sotto classi dell'interfaccia target.
	...
	Creator fornisce il metodo \inline{factory()} che restituisce istanze di Product. L'implementazione di Creator definisce la logica di creazione del'implementazione di Product.

	\item \textbf{Abstract Method}: Come Factory method ma consente di costruire più oggetti tra loro correlati.

	Creator fornisce più metodi \inline{factory()} che restituisce istanze di Product. Le implementazioni di Creator definiscono la logica di creazione delle implementazioni di Product.

\end{itemize}

Nota: Il costruttore deve essere di package.

\bigskip

\textbf{Builder (Oggetto)}: Definisce approccio di creazione step-by-step dell'oggetto.
Scopo: Evitare proliferazione costruttori in una classe.

Vantaggi: Facile creare oggetti immutabili. Esplicito quali parametri sto impostando. Non c'è un ordine in cui specificare i parametri.
...
Una classe Builder ha il costruttore per impostare opzionalmente i pochi parametri obbligatori, dei metodi per settare le proprietà opzionali dell'oggetto target e un metodo build che crea l'istanza dell'oggetto target (e fa controlli). Si adotta un'interfaccia \textit{fluet} (setter restituiscono \inline{this} e il tipo di ritorno è la classe stessa).

Uso se implementata come classe statica innestata.

\oneline{new Classe.Builder([...]).selettori(...).build();}

\medskip

Specializzazione: \inline{StringBuilder} crea dinamicamente strighe immutabili tramite i metodi \inline{append()} e \inline{toString()}.