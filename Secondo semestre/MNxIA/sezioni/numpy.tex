...
\tabella{Campionamento}{
	...
	\riga{np.random.rand()}{Genera numeri uniformemente distribuiti in [o, 1[. Con parametro n si fa un vettore e con parametri n, m un matrice.}
	\riga{np.random.randn()}{Genera numeri dalla distribuzione normale standard. Con parametro n si fa un vettore e con parametri n, m un matrice.}
	...
	\riga{np.random.permutation(arr)}{Restituisce una copia permutata di arr.}
}
\tabella{Parametri creazione}{
...
\tabella{Comparazione}{
	...
	\riga{arr1 condizone arr2}{Da l'array dei booleani valutati elemento per elemento (stesso indice) secondo la condizone. Si può fare il confronto anche con uno scalare. Per condizioni multiple si usano gli operatori \&, \textbar, e le single condizioni vanno tra ().}
}
...
\tabella{Viste}{
	...
	\riga{arr.flatten()}{Restituisce una copia di arr flatterizzata.}
	\riga{arr.ravel()}{Restituisce una view di arr flatterizzata.}
}
Flatterizzare significa appiattire l'array multidimensionale a uno di una sola dimensione.
\tabella{Manipolazione}{
	...
	\riga{arr[[r1, r2]] = arr[[r2, r1]]}{Scambia le righe r1 e r2 di arr.}
...
\tabella{Plt}{
	\riga{plt.plot(x, y, '...')}{Aggiuge una funzione con x input e y funzione da rappresentare con lo stile definito tra apici (r g b..., - * \^, \texttt{--} : ). Si possono aggiungere più funzioni allo stesso grafico.}
	\riga{plt.semilogy(x, y, '...')}{Come plt.plot ma la y è in scala semilogaritmica.}
	\riga{plt.subplot(r, c, p)}{Specifica in quante righe e colonne dividere lo spazio e in quale poszione disegnare il prossimo grafico.}
	\riga{plt.bar(names, values)}{Fa un grafico a barre.}
	\riga{plt.pie(dims, labels=[...], colors=[...])}{Fa un grafico a torta.}
	\riga{plt.grid(True)}{Abilita la visualizzazione della griglia.}
	\riga{plt.xlabel('...')}{Imposta la label dell'asse x.}
	\riga{plt.ylabel('...')}{Imposta la label dell'asse y.}
	\riga{plt.title('...')}{Imposta la label del titolo.}
	\riga{plt.legend(["...", ...])}{Fa la legenda delle funzioni aggiunte.}
	\riga{plt.savefig("nome.estensione")}{Salva la figura.}
	\riga{plt.text(x, y, 'testo', c='...')}{Scrive nella posizione x y il testo specificato.}
	\riga{plt.annotate('testo', xy=(x,y), xytext=(x,y), arrowprops=\{'arrowstyle':'-$>$'\})}{Scrive un annotazione in posizione xytext che punta a xy.}
	\riga{plt.show()}{Mostra la figura.}
}